...
(async () => {
  ...
  await rabbit1.channel.assertExchange(EXCHANGE, "topic", { durable: true });
  await rabbit2.channel.assertExchange(EXCHANGE, "topic", { durable: true });

  await rabbit1.channel.assertQueue(QUEUE_1, { durable: false });
  await rabbit1.channel.assertQueue(QUEUE_2, { durable: false });
  await rabbit2.channel.assertQueue(QUEUE_1, { durable: false });
  await rabbit2.channel.assertQueue(QUEUE_3, { durable: false });

  await rabbit1.channel.bindQueue(QUEUE_1, EXCHANGE, "sandbox"); // load-balance any sandbox
  await rabbit1.channel.bindQueue(QUEUE_2, EXCHANGE, "sandbox.1"); // unique reply to queue
  await rabbit1.channel.bindQueue(QUEUE_2, EXCHANGE, "orange.*"); // subscribe to any event interested
  await rabbit2.channel.bindQueue(QUEUE_1, EXCHANGE, "sandbox"); // load-balance any sandbox
  await rabbit2.channel.bindQueue(QUEUE_3, EXCHANGE, "sandbox.2"); // unique reply to queue
  await rabbit2.channel.bindQueue(QUEUE_3, EXCHANGE, "apple.*"); // subscribe to any event interested
